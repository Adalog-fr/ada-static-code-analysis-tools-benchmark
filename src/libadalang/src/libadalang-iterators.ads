...
package Libadalang.Iterators is
   ...
   function Evaluate
     (P : in out Ada_Node_Predicate_Interface; N : Ada_Node) return Boolean is abstract;
   ...
   type Ada_Node_Predicate_Array is array (Positive range <>) of Ada_Node_Predicate;
   ...
   function Child_With
     (Field     : Syntax_Field_Reference;
      Predicate : Ada_Node_Predicate) return Ada_Node_Predicate;
   ...
   function Find_First
     (Root : Ada_Node'Class; Predicate : Ada_Node_Predicate'Class) return Ada_Node;
   --  Return the first node found under ``Root`` (included) that satisfies the
   --  given ``Predicate``. Return a null node if there is no such node.
...
private
   ...
   type For_All_Predicate (N : Natural) is new Ada_Node_Predicate_Interface with record
   ...
   type For_Some_Predicate (N : Natural) is new Ada_Node_Predicate_Interface with record
      Predicates : Ada_Node_Predicate_Array (1 .. N);
   end record;
   ...
   type For_All_Children_Predicate is new Ada_Node_Predicate_Interface with record
      Predicate : Ada_Node_Predicate;
      Skip_Null : Boolean;
   end record;
   ...
   type For_Some_Children_Predicate is new Ada_Node_Predicate_Interface with record
      Predicate : Ada_Node_Predicate;
      Skip_Null : Boolean;
   end record;
   ...
   type Node_Is_Null_Predicate is new Ada_Node_Predicate_Interface with null record;
