...
(*.[0-9]*)
	...
	if test ! -f "/workspaces/bench-source/src/ncursesada/man_alias.sed" ; then
cat >>/workspaces/bench-source/src/ncursesada/man_alias.sed <<-CF_EOF2
		s,@DATADIR@,$datadir,g
...
s,@TSET@,tset,g
CF_EOF2
		echo "...made /workspaces/bench-source/src/ncursesada/man_alias.sed"
	fi
	...
	aliases=`sed -f "$top_srcdir/man/manlinks.sed" "$inalias" |sed -f "/workspaces/bench-source/src/ncursesada/man_alias.sed" | sort -u; test "$inalias" = "$nCurses" && echo curses`
	cf_target=`grep "^$cf_source" /workspaces/bench-source/src/ncursesada/man/man_db.renames | mawk '{print $2}'`
	...
	sed	-f "/workspaces/bench-source/src/ncursesada/man_alias.sed" \
		< "$i" | sed -f /workspaces/bench-source/src/ncursesada/edit_man.sed >$TMP
