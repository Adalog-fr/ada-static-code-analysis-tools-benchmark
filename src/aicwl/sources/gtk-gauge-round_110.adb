...
package body Gtk.Gauge.Round_110 is

   Needle_Color      : constant Gdk_Color := RGB (1.0, 0.0, 0.0);
   Background_Color  : constant Gdk_Color := RGB (0.0, 0.0, 0.0);
   Area_Color        : constant Gdk_Color := RGB (0.0, 0.2, 0.3);
   Line_Color        : constant Gdk_Color := RGB (0.8, 1.0, 1.0);
   Major_Tick_Color  : constant Gdk_Color := RGB (1.0, 1.0, 1.0);
   Middle_Tick_Color : constant Gdk_Color := RGB (1.0, 1.0, 1.0);
   Minor_Tick_Color  : constant Gdk_Color := RGB (1.0, 1.0, 1.0);
   Text_Color        : constant Gdk_Color := RGB (1.0, 1.0, 1.0);

   Class_Record : aliased Ada_GObject_Class := Uninitialized_Class;

   Aspect_Ratio : constant := 3.7;

   Length : constant := 110.0 * Pi / 180.0;
   First  : constant := (180.0 + 35.0) * Pi / 180.0;
   Offset : constant := 15.0 * Pi / 180.0;
   Span   : constant := Length - Offset * 2.0;
   Excess : constant := 1.42 * Pi / 30.0;
   X : constant := 0.0;
   Y : constant := 0.48;
   ...
   begin
      ...
      Widget.Arrows_Area :=
         ...
         (  Under         => Widget.Background.Get_Foreground,
            Color         => Area_Color,
            Outer         => ((X, Y), 1.0 / 0.518, 0.518, 0.0),
            From          => First  + Excess,
            Length        => Length - Excess * 2.0,
            Border_Depth  => 0.005,
            Border_Color  => (False, Area_Color),
      ...
      Widget.Line_2 :=
         Add_Arc
         (  Under   => Widget.Background.Get_Foreground,
            Ellipse => ((X, Y), 1.0 / 0.485, 0.485, 0.0),
            Color   => Line_Color,
            Width   => 1.0 / 400.0,
            From    => First + Offset,
            Length  => Span,
      ...
      Widget.Minor_Ticks :=
         Add_Elliptic_Scale
         (  Under   => Widget.Background.Get_Foreground,
            Inner   => ((X, Y), 1.0 / 0.470, 0.470, 0.0),
            Outer   => ((X, Y), 1.0 / 0.485, 0.485, 0.0),
            Color   => Minor_Tick_Color,
            Width   => 1.0 / 400.0,
            Skipped => 5,
            Step    => Span / (10.0 * GDouble (Widget.Sectors)),
            From    => First  + Offset,
            Length  => Span,
            Scaled  => True,
            Widened => True
      ...
      Widget.Middle_Ticks :=
         Add_Elliptic_Scale
         (  Under   => Widget.Background.Get_Foreground,
            Inner   => ((X, Y), 1.0 / 0.47, 0.47, 0.0),
            Outer   => ((X, Y), 1.0 / 0.50, 0.50, 0.0),
            Color   => Middle_Tick_Color,
            Width   => 1.0 / 400.0,
            Skipped => 2,
            Step    => Span / (2.0 * GDouble (Widget.Sectors)),
            From    => First  + Offset,
            Length  => Span,
            Scaled  => True,
            Widened => True
      ...
      Widget.Major_Ticks :=
         ...
         (  Under   => Widget.Background.Get_Foreground,
            ...
            Width   => 2.0 / 400.0,
            Step    => Span / GDouble (Widget.Sectors),
            From    => First  + Offset,
            Length  => Span,
            Scaled  => True,
            Widened => True
   ...
   function Get_Annotation
            (  Widget : not null access Gtk_Gauge_Round_110_Record
      ...
      then
         ...
         (  Class_Ref (Class_Record.The_Type),
            ...
            (  Name       => "line-color",
               Boxed_Type => Gdk_Color_Type,
               Nick       => "Line color",
               Blurb => "The color of the circles bounding the ticks"
         )  );
         Install_Style_Property
         (  Class_Ref (Class_Record.The_Type),
            Gnew_Boxed
   ...
   begin
      Widget := new Gtk_Gauge_Round_110_Record;
