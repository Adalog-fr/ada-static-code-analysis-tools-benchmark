...
with AdaCL.Trace;
with Create_LED_Test.Suite;

procedure Create_LED_Test.Main is
   Trace : AdaCL.Trace.Object := AdaCL.Trace.Function_Trace;
   pragma Unreferenced (Trace);

   use type AUnit.Status;

   function Run is new AUnit.Run.Test_Runner_With_Status (Create_LED_Test.Suite.Suite);

   Reporter   : AUnit.Reporter.Text.Text_Reporter;
   Status     : constant AUnit.Status := Run (Reporter);
   Trace_File : constant String       := "create_led_test.out";
begin
