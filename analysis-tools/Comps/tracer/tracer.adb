...
package body Tracer is
   ...
   -- user (see file tracer_implementation.html for a full discussion
   -- of this issue).
   --
   ...
   -- SECTION AUTODEBUG:
   --   Some procedures used to trace Tracer itself, or for the diagnosis
   --   of internal failures.
   -- SECTION SERVICES:
   --   internal utilities, not exported by Tracer
   -- SECTION PROVIDED SUBPROGRAMS:
   --   implementation of subprograms provided in the specification of Tracer
   ...
   type Tracer_Mode is (Silent, Mono_Task, Multi_Task);
   ...
   --
   --  The following are variables controlling the behaviour of Tracer.
   ...
   Timer_Id : Task_Id;
   Proxy_Id : Task_Id;
   -- ID's of Tracer's internal tasks.
   ...
   -- running, but they cannot call operations from Tracer).

   Tracer_File_Input : Boolean := False;
   --  True if Tracer reads input from a file rather than the keyboard.
   ...
   Tracer_Output : Ada.Text_Io.File_Access := Ada.Text_Io.Current_Error;
   ...
   --  execution. Pointed at by Tracer_Output.
   ...
   Current_Mode : Tracer_Mode := Mono_Task;
   ...
   --                           SECTION AUTODEBUG                            --
   --                                                                        --
   -- This section contains internal utilities for debugging Tracer, and     --
   -- managing internal Tracer errors.                                       --
   --                                                                        --
   -- These procedures (and all calls to them) can be removed from a         --
   -- working version. But after all, the whole package Tracer could be      --
   ...
   --  Called when an unexpected exception is raised within Tracer
   --

   procedure Tracer_Error (Problem : Ada.Exceptions.Exception_Occurrence;
                           Where   : String)
   is
   ...
   begin
      ...
      Set_Col (Tracer_Output.all, 1);
      Put (Tracer_Output.all, Tracer_Message);
      Put (Tracer_Output.all, Bug_Message);
      Put (Tracer_Output.all, Where);
      Put (Tracer_Output.all, " : ");
      Put (Tracer_Output.all, Exception_Name (Problem));
      New_Line (Tracer_Output.all);
      if Message /= "" then
         Put (Tracer_Output.all, Tracer_Message);
         Put (Tracer_Output.all, "   ");
         Put (Tracer_Output.all, Bug_Mess_Message);
         Put (Tracer_Output.all, Message);
         New_Line (Tracer_Output.all);
      end if;
      if Information /= "" then
         Put (Tracer_Output.all, Tracer_Message);
         Put (Tracer_Output.all, "   ");
         Put (Tracer_Output.all, Bug_Info_Message);
         Put (Tracer_Output.all, Information);
         New_Line (Tracer_Output.all);
      end if;

      --  Reraise the exception, so we will get the stack trace
      --  (All calling procedures will call Tracer_Error in turn)
   ...
   --  other Tracer facilities. Leave it to True, unless you are in a situation
   ...
   procedure Itrace (Message : String) is
      ...
      -- Normally, the same as Tracer_Output, but it may be convenient to
      -- have Itrace output different from "regular" Tracer output
      -- (f.e. Standard_Output). Change the following constant if needed
      Itrace_Output : constant File_Access := Tracer_Output;
   begin
   ...
   begin   --  Timer
      Timer_Id := Timer'Identity;  --  Note ID's of Tracer's tasks
   ...
   -- It is not possible to exit silent mode, since all calls to Tracer
   ...
   --
   --  Check whether a task is one of Tracer internal tasks
   --  (these tasks should never appear to the user)
   --

   function Is_Tracer_Task (Id : Task_Id) return Boolean is
   begin
   ...
   begin
      ...
      elsif Is_Tracer_Task (Caller) then
   ...
   begin
      --  Output Trace_Message if we are at the start of a new message
      if New_Message then
         Put (Current_Error, Tracer_Message);
         New_Message := False;
      end if;
   ...
   begin  --  Print
      if Target /= None then
         ...
         if Current_Mode = Mono_Task and then Is_Tracer_Task (Source_Id) then
            --  Pretend the task is main task to avoid entering multi-task mode
            Id := Main_Task;
         end if;

         if Id /= Previous_Task and then
            not (Is_Tracer_Task (Id) and then Is_Tracer_Task (Previous_Task))
            --  Do not show switches between internal (hidden) tasks
         then
            if Current_Mode = Mono_Task then
               Put_Line (Tracer_Output.all, Tracer_Message      &
                                           Multi_Task_Message &
                                           Image (Main_Task));
               Current_Mode := Multi_Task;
            end if;
            Put_Line (Tracer_Output.all, Tracer_Message & Task_Separator);
   ...
   exception
      when Occur : others =>
         Tracer_Error (Occur, "Safe_Open");

         --  The following statement will never be executed, since Tracer_Error
   ...
   exception
      when Occur : others =>
         Tracer_Error (Occur, "Tracer finalization");
   end Finalize_Tracer;
   ...
   procedure Execute_Commands (Current_Message : Message_Access) is
      ...
      begin
         case To_Display is
            ...
            when Task_Status =>
               if Is_Tracer_Task (Selected_Task) then
   ...
   Over_Messages :
      ...
      Over_Tags : --  Loop over tags in message
         while Cur_Pos < Line_Length loop
            Cur_Pos := Cur_Pos + 1;
            --  Skip separators
            while Input_Line (Cur_Pos) = ' '
              or  Input_Line (Cur_Pos) = Ascii.Ht
              or  Input_Line (Cur_Pos) = ','
            loop
            ...
            case C is
               ...
               when Mark_Char =>
                  if not Is_Tracer_Task (Selected_Task) then
   ...
   exception
      when Occur : others =>
         Tracer_Error (Occur, "Execute_Commands");
   ...
   exception         --  Safety exception handler
      when Occur : others =>
         Tracer_Error (Occur, "Unprotected_Flush");
   ...
   begin
      case To is
         when File =>
            ...
            Tracer_Output := Trace_File'Access;
         when Console =>
            Tracer_Output := Current_Error;
         when None =>
            null;
      end case;
   ...
   exception
      when Closed_Semaphore =>
         null;
      when Occur : others =>
         Tracer_Error (Occur, "Flush_Trace");
   end Flush_Trace;
   ...
   exception
      when Tasking_Error => --  Suspend_Timer called after Abort_Main
         null;              --  Ignore it
      when Occur : others =>
         Tracer_Error (Occur, "Suspend_Timer");
   end Suspend_Timer;
   ...
   exception
      when Tasking_Error => --  Resume_Timer called after Abort_Main
         null;              --  Ignore it
      when Occur : others =>
         Tracer_Error (Occur, "Resume_Timer");
   end Resume_Timer;
   ...
   -------------------------------------------------------------------------
   --  Initialization code for Tracer services
   -------------------------------------------------------------------------

   package Tracer_Last_Will is new Last_Will (Finalize_Tracer) with Unreferenced;
...
exception
   when Occur : others =>
      Tracer_Error (Occur, Bug_Tracer_Init);
end Tracer;
