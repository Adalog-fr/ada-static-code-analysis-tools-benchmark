      ...
      case Cat is
         when Discrete_Types | An_Access_Type =>
            if Is_Nil (Initialization_Expression (Decl)) then
               declare
                  function Decl_Default_Value return Extended_Biggest_Int is
                  -- Made a function of this sequence because having "return" is more readable
                  -- than complicated "elsif"
                  ...
                  begin
                     ...
                     Type_Decl := Corresponding_Declaration_Type_Definition (Decl);
                     if Is_Nil (Type_Decl) then
                        return Not_Static;
                     end if;
                     Type_Decl := Enclosing_Element (Type_Decl);
                     if Declaration_Kind (Type_Decl) in A_Type_Declaration then
                        declare
                           Aspects : constant Asis.Definition_List
                                        := Corresponding_Aspects (Type_Decl, "DEFAULT_VALUE");
                        begin
                           if Aspects /= Nil_Element_List then
                              return Discrete_Static_Expression_Value (Aspect_Definition ( Aspects (1)));
                           end if;
                        end;
                     end if;
                     if Cat = An_Access_Type then
                        -- not a parameter, no default aspect:
                        -- without anything else, access variables are initialized to null
                        return 0;
                     end if;
                     return Not_Static;
                  end Decl_Default_Value;
               begin
                  Initial_Value := Decl_Default_Value;
               end;
            end if;
            for Name : Asis.Name of Names (Decl) loop
               if not Is_Nil (Initialization_Expression (Decl)) then
