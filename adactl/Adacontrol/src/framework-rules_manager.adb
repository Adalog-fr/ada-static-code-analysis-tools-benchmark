...
package body Framework.Rules_Manager is
   ...
   procedure Accumulate_Time is
      use Ada.Calendar;
      use Rule_List;
      Rule_Name : constant Unbounded_Wide_String := To_Unbounded_Wide_String (Last_Rule);
   ...
   procedure Enter (Rule : Wide_String) is
      -- Can be called with Rule=No_Rule ("") to mean no more rule is active (stop timers)
      use Ada.Calendar;
   begin
  ...
  begin
   ...
   begin  -- Report_Timings
      case Timing_Option.Value is
         when Off =>
            return;
         when On =>
            if Global_Report then
               return;
            end if;
         when Global =>
