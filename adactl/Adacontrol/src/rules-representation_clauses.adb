...
--  of Air Navigation (EUROCONTROL) and Adalog 2004-2023.           --
...
package body Rules.Representation_Clauses is
   ...
   -- Subrule => Context
   type Restriction is (Unspecified, Excluded, Required);
   Report_Truth_Table : constant array (Boolean, Restriction) of Boolean :=
     (False => (Unspecified => True, Excluded => True,  Required => False),
      True  => (Unspecified => True, Excluded => False, Required => True));
   ...
   begin
      ...
      Help_On_Flags (Header      => "Parameter(s): [<category>]",
                     Footer      => "(optional)",
                     Extra_Value => "[[not] global] [[not] object] <attribute>");
      ...
      if Parameter_Exists then
         while Parameter_Exists loop
            ...
            if Subrule = Sr_Attribute then
               Has_Not_G := Get_Modifier ("NOT");
               Global    := Get_Modifier ("GLOBAL");
               if not Global then
                  Has_Not_O := Has_Not_G;
               else
                  Has_Not_O := Get_Modifier ("NOT");
               end if;
               Object := Get_Modifier ("OBJECT");
            ...
            begin
               Associate (Usage,
                          Param,
                          Repr_Context' (Basic.New_Context (Ctl_Kind, Ctl_Label) with
                            Cats,
                            (if Global then (if Has_Not_G then Excluded else Required) else Unspecified),
                            (if Object then (if Has_Not_O then Excluded else Required) else Unspecified)),
                          Additive => True);
            exception
         ...
         Associate (Usage, Key_All, Repr_Context'(Basic.New_Context (Ctl_Kind, Ctl_Label)
                                    with Full_Set, Global => Unspecified, Object => Unspecified));
   ...
   procedure Process_Clause (Rep_Clause : in Asis.Representation_Clause) is
      ...
      begin  -- Do_Report
         ...
         while not Is_Exhausted (Iter) loop
            ...
            if    Report_Truth_Table (Is_Object, Cont.Object)
              and Report_Truth_Table (Is_Global, Cont.Global)
            then
               if Cont.Cats = Empty_Set then
                  Report (Rule_Id,
                          ...
                          (case Cont.Global is
                              when Unspecified => "",
                              when Excluded    => "not global ",
                              when Required    => "global ")
                          &
                          (case Cont.Object is
                                when Unspecified => "",
                                when Excluded    => "not object ",
