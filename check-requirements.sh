...
check_python_version() {
    ...
    if [[ "$major" -ge 3 && "$minor" -ge 10 ]]; then
        ...
        print_error "Python version $version is lower than 3.10 ✗"
        return 1
    fi
}
...
check_sed_version() {
    if check_command sed; then
        ...
        if [[ $(echo -e "$version\n4.9" | sort -V | head -n1) != "4.9" ]]; then
            print_error "sed version $version is lower than 4.9 ✗"
            ...
            print_success "sed version $version ✓"
            return 0
        fi
    else
...
check_deno_version() {
    if check_command deno; then
        ...
        if [[ "$major" -lt 2 ]]; then
            print_success "deno version $version ✓"
            return 0
        else
            print_error "deno version $version is greater than or equal to 2.0.0, bugs may occur ✗"
...
check_alr() {
    if check_command alr; then
        print_success "alr is installed ✓"
        return 0
    else
        print_error "alr is not installed ✗"
        if confirm "Do you want to install alr?"; then
            if check_command deno; then
                if [[ -f "install/install_ada_setup.ts" ]]; then
                    ...
                    if check_command alr; then
                        print_success "alr has been successfully installed ✓"
                        return 0
                    else
                        print_error "alr installation failed ✗"
                        return 1
                    fi
                else
...
check_adactl() {
    if [[ -x "analysis-tools/Adacontrol/bin/adactl" ]]; then
        print_success "adactl is present ✓"
        return 0
    else
        print_error "adactl is not present in analysis-tools/Adacontrol/bin/ ✗"
        if confirm "Do you want to compile it?"; then
            if check_command alr; then
                print_info "Compiling adactl..."
                (cd analysis-tools/Adacontrol && alr build)
                if [[ -x "analysis-tools/Adacontrol/bin/adactl" ]]; then
                    print_success "adactl has been successfully compiled ✓"
                    return 0
                else
                    print_error "adactl compilation failed ✗"
                    return 1
                fi
            else
...
check_atgdb() {
    if [[ -x "analysis-tools/cogralys-engine/bin/atgdb" ]]; then
        print_success "atgdb is present ✓"
        return 0
    else
        print_error "atgdb is not present in analysis-tools/cogralys-engine/bin/ ✗"
        if confirm "Do you want to compile it?"; then
            if check_command alr; then
                print_info "Compiling atgdb..."
                (cd analysis-tools/cogralys-engine && alr build)
                if [[ -x "analysis-tools/cogralys-engine/bin/atgdb" ]]; then
                    print_success "atgdb has been successfully compiled ✓"
                    return 0
                else
                    print_error "atgdb compilation failed ✗"
                    return 1
                fi
            else
...
check_requirements() {
    ...
    if check_command scc; then
        print_success "SCC is installed ✓"
    else
        print_error "SCC is not installed ✗"
        requirements_met=1
    fi

    # Check typst
    if check_command typst; then
        print_success "typst is installed ✓"
    else
        print_warning "typst is not installed, it is required to generate PDF reports"
        requirements_met=1
    fi
    ...
    if check_command gnatcheck; then
        print_success "gnatcheck is installed ✓"
    else
        print_error "gnatcheck is not installed ✗"
        requirements_met=1
    fi

    # Check gnatls
    if check_command gnatls; then
        print_success "gnatls is installed ✓"
    else
        print_error "gnatls is not installed ✗"
        requirements_met=1
    fi

    # Check cypher-shell
    if check_command cypher-shell; then
        print_success "cypher-shell is installed ✓"
    else
        print_error "cypher-shell is not installed ✗"
        requirements_met=1
    fi
    ...
    if check_command jc; then
        print_success "jc is installed ✓"
    else
        print_error "jc is not installed ✗"
        requirements_met=1
    fi

    # Check parallel
    if check_command parallel; then
        print_success "parallel is installed ✓"
    else
        print_error "parallel is not installed ✗"
        requirements_met=1
    fi
    ...
    if check_command fd; then
        ...
        print_success "fdfind is installed ✓"
    else
        print_error "fd/fdfind is not installed ✗"
        if confirm "Do you want to install fd-find?"; then
            sudo apt-get install fd-find
            if check_command fdfind; then
                print_success "fd-find has been successfully installed ✓"
            else
                print_error "fd-find installation failed ✗"
                requirements_met=1
            fi
        else
            requirements_met=1
